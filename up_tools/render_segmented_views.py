...
def render(model, resolution, cam, steps, segmented=False, use_light=False):  # pylint: disable=too-many-arguments
    ...
    for angle in _np.linspace(0., 2. * (1. - 1. / steps) * _np.pi, steps):
        mean = _np.mean(_np.array(base_mesh.v), axis=0, keepdims=True)
        # (_np.array(base_mesh.v)-mean)
        mesh.v = _rotateY((_np.array(base_mesh.v)-mean),angle) + mean
        from write_mesh import write
        # write('/home/harryh/nbf/demo/up/output/images/'+str(angle)+'.obj', mesh.v, mesh.f)
        write('/home/harryh/nbf/demo/up/output/images/'+str(datetime.datetime.now())+'_'+str(angle)+'.obj', mesh.v, mesh.f)
        ...
        im[:h, :w, :] = imtmp*255.
        print('angle', angle)
        # _cv2.imshow('win', _np.array(im).astype(_np.uint8))
        # _cv2.waitKey()
        _cv2.imwrite('/home/harryh/nbf/demo/up/output/images/'+str(datetime.datetime.now())+'_'+str(angle)+'.png', _np.array(im).astype(_np.uint8))
